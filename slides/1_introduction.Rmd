...
- Many interesting and important questions cannot be answered in a randomized trial: We need theory and language that allows us to test causal hypotheses in observational data. 
...
+ Many equivalent notations:
  ...
  - $Y\vert \ do(A = a)$: This notation explicitly emphasizes the action of setting $A$ equal to $a$ and is favored by Judea Pearl. 
...
+ Let $y_i$ and $a_i$ be the observed values of $Y_i$ and $A_i$. 

+ Consistency states that $y_i = Y_i(A_i = a_i)$.

+ The observed value of $Y_i$ is the same as the counterfactual value of $Y_i$ under the treatment $a_i$. 
...
+ Conditional exchangeability allows us to recover identification of ATE with additional measured features. 

+ Conditional exchangeability holds with respect to a set of variables $X$, if
...
+ The positivity condition states that at all individuals have some chance of recieving any treatment: 

$$P[A = a \vert X = x] > 0 \ \ \forall a, x$$
...
Theorem: If consistency, SUTVA, conditional exchangeability, *and positivity* hold, then $$Y(a) \vert X =x \overset{d}{=} Y | X = x, A = a.$$ 
...
# Identifying a Causal Effect in an Observational Study
