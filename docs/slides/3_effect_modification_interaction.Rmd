...
date: "2022-01-19 (updated: `r Sys.Date()`)"
output:
...
p_v0 <- c(1, 3,4, 2) #Prob of (0, 0), (0, 1), (1, 0), (1, 1)
p_v1 <- c(1, 5, 2, 2)

cat_v0 <- rep(1:4, p_v0) %>% sample(size = 10)

cat_v1 <- rep(1:4, p_v1) %>% sample(size = 10)
cat <- c(cat_v0, cat_v1)
Y0 <- Y1 <- rep(0, n)
Y0[cat %in% c(3, 4)] <- 1
Y1[cat %in% c(2, 4)] <- 1

X<- data.frame(Ind = 1:20, Y0 = Y0, Y1 = Y1, V = V) 
  #head() %>%
kable(X[1:10,], col.names = c("", "\\(Y(A=0)\\)", "\\(Y(A=1)\\)", "\\(V\\)"), format = 'html') %>%
...
kable(X[11:20,], col.names = c("", "\\(Y(A=0)\\)", "\\(Y(A=1)\\)", "\\(V\\)"), format = 'html', row.names = FALSE) %>%
...
$$E[Y(1) \vert V = 10] - E[Y(0) \vert V = 1] = `r round(mean(Y1[V ==1]), digits = 2)` - `r round(mean(Y0[V == 1]), digits = 2)` = `r round(mean((Y1-Y0)[V==1]), digits = 2)`$$
...
- If 
  ...
  then $V$ <u>always</u> modifies the effect of $A$ on $Y$ on either the additive or multiplicative scale (or both).
...
ex_node <- create_node_df(n = 4, label = c("V", "A", "Y", "L"), 
                     ...
                     x = c(1, 0, 1, 0.5), 
                     y = c(1, 0, 0, 0.8))
ex_edge <- create_edge_df(from = c(1, 2, 4, 4),
                          to = c(3, 3, 2, 3),
                          minlen = 1, 
                          color = "black", 
                          )
...
Xd <- X[1:20,] %>% arrange(A, L, V)


kable(Xd[1:10,], col.names = c("\\(Y(0)\\)", "\\(Y(1)\\)", "\\(A\\)", "\\(L\\)", "\\(V\\)"), format = 'html', row.names = TRUE) %>%
  kable_styling(position = "float_left", full_width = FALSE) %>%
   column_spec(2, bold = ifelse(Xd$A[1:10] == 0, TRUE, FALSE), 
              color = ifelse(Xd$A[1:10] == 0, "black", "grey")) %>% 
   column_spec(3, bold = ifelse(Xd$A[1:10] == 1, TRUE, FALSE), 
              color = ifelse(Xd$A[1:10] == 1, "black", "grey"))

kable(Xd[11:20,], col.names = c( "\\(Y(0)\\)", "\\(Y(1)\\)", "\\(A\\)", "\\(L\\)", "\\(V\\)"), format = 'html') %>%
  kable_styling(position = "right", full_width = FALSE) %>%
   column_spec(2, bold = ifelse(Xd$A[11:20] == 0, TRUE, FALSE), 
              color = ifelse(Xd$A[11:20] == 0, "black", "grey")) %>% 
   column_spec(3, bold = ifelse(Xd$A[11:20] == 1, TRUE, FALSE), 
              color = ifelse(Xd$A[11:20] == 1, "black", "grey"))
...
If we know the full counterfactuals, we can compute the conditional effects directly
...
kable(Ta[Ta$A==0,], col.names = c("\\(L\\)", "\\(V\\)","\\(A\\)", "\\(N\\)", "\\(\\bar{Y}\\)"), format = 'html', row.names =FALSE) %>%
  kable_styling(position = "float_left", full_width = FALSE)

kable(Ta[Ta$A==1,], col.names = c("\\(L\\)", "\\(V\\)","\\(A\\)", "\\(N\\)", "\\(\\bar{Y}\\)"), format = 'html', row.names =FALSE)%>%
  kable_styling(position = "center", full_width = FALSE)
```

$$ P[L = 0 \vert V = 0] = \frac{`r S$N[1]`}{`r S$N[1] + S$N[3]`} = `r round(S$N[1]/( S$N[1] + S$N[3]), digits = 2)` \qquad P[L = 0 \vert V = 1] = \frac{`r S$N[2]`}{`r S$N[2] + S$N[4]`} = `r round(S$N[2]/( S$N[2] + S$N[4]), digits = 2)`$$ 

$$E[Y(1) \vert V = 0] = `r Ta$Y[2]`\cdot `r plgivenv$pr[1]` + `r Ta$Y[6]`\cdot `r plgivenv$pr[3]` = `r round(Ta$Y[2]* plgivenv$p[1] + Ta$Y[6]*plgivenv$p[3], digits = 2)`$$

$$E[Y(0) \vert V = 0] = `r Ta$Y[1]`\cdot `r plgivenv$pr[1]` + `r Ta$Y[5]`\cdot `r plgivenv$pr[3]` = `r round(Ta$Y[1]* plgivenv$p[1] + Ta$Y[5]*plgivenv$p[3], digits = 2)`$$

$$E[Y(1) \vert V = 0]-E[Y(0) \vert V = 0] = `r round((Ta$Y[2]* plgivenv$p[1] + Ta$Y[6]*plgivenv$p[3])-(Ta$Y[1]* plgivenv$p[1] + Ta$Y[5]*plgivenv$p[3]), digits = 2)`$$
