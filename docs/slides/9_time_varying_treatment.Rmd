...
My interpretation of the proposed method: Propose the following model:

$$E[Y_i \vert A_i, L_i, \hat{\pi}_i] = \beta_0 + \beta_1 A_i + L_i^\top \beta_L + \sum_{j = 1}^{4} \alpha_j I(\hat{\pi}_i \in C_j)$$
where $C_j$ are quartiles of $\hat{\boldsymbol{\pi}}$ and $L$ are a set of $K$ covaraites. 
...
- The ATE is the $\int_0^1 \beta(\pi) d\pi$. 
...
- We approximate $\int_{0}^1 \beta(\pi)d\pi$ with $\sum_{i=1}^{N} \hat{\beta}(\hat{\pi}_i)$ (i.e. standardization).
...
- In the version with no interactions, we have the following model:
...
- In the version with an interaction, we model

$$\tilde{\beta}(L_{1,i}, \hat{\pi}_i) = \sum_{j = 1}^{K} \left(\beta_{1,j} + \beta_{2,j}L_{1,i}\right)I(\hat{\pi}_i \in C_j)$$
---

# Subclassification Regression Estimator

$$\tilde{\mu}(L_i, \hat{\pi}_i) =  \sum_{j = 1}^{K}\left(\beta_{0,j} + L_i^\top \beta_{L,j} \right)I(\hat{\pi}_i \in \hat{C}_j)\\\
\tilde{\beta}( \hat{\pi}_i) =\sum_{j = 1}^{K} \left(\beta_{1,j} + \beta_{2,j}L_{1,i}\right)I(\hat{\pi}_i \in C_j)$$
...
- In our previous framework, our model is $$\tilde{\mu}(L_i) = \beta_0 + L_i^\top \beta_L + \sum_{j = 1}^{4} \alpha_j I(\hat{\pi}_i \in C_j)\\\ \tilde{\beta}(L_i) = \beta_1$$
...
sim1 <- function(n = 1000){
  ...
  Y <- mu + A*beta + rnorm(n = 1000, sd = 0.5)
  z <- log(p/(1-p))
  ...
  dat <- data.frame("A" = A, "Y"=Y, "EY1" = mu + beta, "EY0" = mu, "p" = p, "phat" = fp$fitted.values) %>%
        mutate(wt = A/phat + (1-A)/(1-phat), 
               w1 = A/phat, w2 = (1-A)/(1-phat), pinv = 1/p, pm1inv = 1/(1-phat))
  ...
  badj3 <- mean(dat$v1)-mean(dat$v0)

  raw <- lm(Y ~ A, data = dat)$coefficients[2] %>% as.numeric()
  ests <- c("hj" = as.numeric(fhj$coefficients[2]), "adj1" = badj1, "adj3" = badj3, "raw" = raw, "sate" = mean(beta))
  return(ests)
}
