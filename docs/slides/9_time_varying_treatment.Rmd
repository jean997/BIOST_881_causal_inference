...
$$E[Y_{k} \vert L_{0}, \bar{A}_{k}] = \beta_{0,j} + \beta_{1}^{\top}L_{0} + \beta_2^\top h\left(\sum_{j =0}^k A_{j}\right)$$ 
...
- Using our marginal structural model, we could use our previous IP weighting strategy to compute the causal effect of time on HAART on CD4 count at time $k$. 
...
---
# Iterated Conditional Expectation (ICE) estimation

- As an alternative to the G-formula format we have seen, we could characterize it as a nested expectation

$$E[Y(\bar{a})] = E \left[E\left[\dots E\left[ Y(\bar{a}) \vert \bar{L}(\bar{a})_K\right] \vert \bar{L}(\bar{a})_{K-1} \right] \dots \vert \bar{L}(\bar{a})_0 \right]$$

- For just two time points this is
$$E[Y(a_0, a_1)] = E\left[ E\left[Y(a_0, a_1) \vert L_1(a_0) \right] \vert L_0\right] $$

---
# ICE Estimation

- The nested expectation formulation gives us an alternate estimator.

- Define

$$Q_{K+2} = Y\\\
Q_{K+1} = E[Y \vert\bar{L}_{K}, \bar{A}_{K} = \bar{a}]\\\
\vdots\\\
Q_m = E[Q_{m+1} \vert \bar{L}_{m-1}, \bar{A}_{m-1} = \bar{a}_{m-1}]\\\
\vdots\\\
Q_0 = E[Q_1 \vert L_0, A_0 = a_0]$$

- The idea will be to propose a model for $E[Q_{m+1} \vert \bar{L}_{m-1}, \bar{A}_{m-1} = \bar{a}_{m-1}]$ and then use it iteratively to estimate each $Q$. 

- When we get all the way down to 0, we will have an estimator of $Y(\bar{a})$. 

---
# ICE Estimation

- Example with two time-points: Suppose we are interested in a static intervention $\bar{a} = (a_0, a_1, a_2)$.

- First we propose a model:

$$E[Y\vert \bar{L}_{2}, \bar{A}_{2}] = \theta_{0, 2} + \theta_{1,2}cum(\bar{A}_{2}) + \theta_{2,2} L_{2}$$
- We start at time $m = 2$ and estimate parameters $\hat{\theta}_{0,2}, \hat{\theta}_{1,2}$ and $\hat{\theta}_{2,2}$.

- Next we estimate $\hat{Q}_{2,i}(a_2)$ by plugging $a_2$ for $A_{2,i}$ but leaving the preceding treatment course unaltered. 

---
# ICE Estimation

- Now we repeat, except we use $\hat{Q}_2(a_2)$ as the outcome in our regression

$$E[\hat{Q}_2(a_2) \vert \bar{L}_{1}, \bar{A}_{1}] = \theta_{0, 1} + \theta_{1,1}cum(\bar{A}_1) + \theta_{2,1} L_{1}$$

estimating parameters $\hat{\theta}_{0,1}, \hat{\theta}_{1,1}$ and $\hat{\theta}_{2,1}$

- We estimate $\hat{Q}_{1,i}(a_1,a_2)$ by plugging $a_1$ for $A_{1,i}$ but leaving $A_{0,i}$ at its natural value. 

---
# ICE Estimation

- Finally we fit the regression one last time

$$E[\hat{Q}_1(a_2, a_1) \vert L_{0}, \bar{A}_{0}] = \theta_{0, 0} + \theta_{1,0}cum(\bar{A}_0) + \theta_{2,0} L_{0}$$
estimating parameters $\hat{\theta}_{0,1}, \hat{\theta}_{1,1}$ and $\hat{\theta}_{2,1}$

- We estimate $\hat{Q}_{0,i}(a_0,a_1, a_2)$ by splugging $a_0$ for $A_{0,i}$.

- Now we can compute $E[Y(0)] = \frac{1}{N}\hat{Q}_{0,1}(\bar{a})$
...
- Recall the Scharfstein (1999) DR estimation strategy: 
...
$$\frac{1}{N}\sum_{i=1}^N \left(m(1, L_i; \hat{\theta}) + \hat{\phi_1}\frac{1}{\hat{\pi}_i} \right) - \frac{1}{N}\sum_{i=1}^N \left(m(0, L_i; \hat{\theta}) + \hat{\phi_2}\frac{1}{1-\hat{\pi}_i} \right)$$
---
