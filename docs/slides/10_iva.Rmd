...
run_one <- function(n, comp =  c(0.85, 0.06, 0.08, 0.03)){
  df <- data.frame(Q = sample(1:4, size = n, replace = TRUE, prob = comp), 
                   Z = rep(c(0, 1), each =n/2)) %>%
        mutate(A = case_when(Z == 0 & (Q == 1 | Q == 3) ~ 0,
                             Z == 0 & (Q == 2 | Q == 4) ~ 0,
                             Z == 1 & (Q == 1 | Q == 2) ~ 1,
                             Z == 1 & (Q == 3 | Q == 4) ~ 0),
               EY = case_when(A == 1 ~ 3,
...
```{r, echo=FALSE, eval = FALSE}
run_one <- function(n, comp =  c(0.85, 0.06, 0.08, 0.03), ATT = 3, ATU = 1.5){
  df <- data.frame(Q = sample(1:4, size = n, replace = TRUE, prob = comp), 
                   Z <- rep(c(0, 1), each =n/2)) %>%
        mutate(A = case_when(Z == 0 & (Q == 1 | Q == 3) ~ 0,
                             Z == 0 & (Q == 2 | Q == 4) ~ 0,
                             ...
                             Z == 1 & (Q == 3 | Q == 4) ~ 0),
               EY0 = rnorm(n = n), 
               EY1 = EY0 + ATT*A + rnorm(n = n, sd= 0.5))
