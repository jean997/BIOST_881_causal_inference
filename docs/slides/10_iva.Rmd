...
Z <- rnorm(n)
U <- rnorm(n) + 1
A <- 0.2 + 0.5*Z + Z^2-3*U + rnorm(n)
Y <- 2*A - 5*U + rnorm(n = n)
df <- data.frame(Z = Z, U = U, A = A, Y = Y)


#2SLS
f1 <- lm(A ~ Z + I(Z^2), data = df)
df <- df %>% mutate(Ahat = f1$coefficients[2]*Z + f1$coefficients[3]*Z^2)
f2 <- lm(Y ~ Ahat, data = df)
bhat2 <- f2$coefficients[2]
# Ratio
faz1 <- lm(A ~ Z, data = df)
faz2 <- lm(A ~ I(Z^2), data = df)
fyz1 <- lm(Y ~ Z , data = df)
fyz2 <- lm(Y ~ I(Z^2), data = df)
by <- c(fyz1$coefficients[2], fyz2$coefficients[2])
ba <- c(faz1$coefficients[2], faz2$coefficients[2])
sy <- c(sqrt(vcov(fyz1)[2,2]), sqrt(vcov(fyz2)[2,2]))
sa <- c(sqrt(vcov(faz1)[2,2]), sqrt(vcov(faz2)[2,2]))
br <- by/ba
w <- ba^2/sy^2
bhatR <- sum(w*br)/sum(w)
```
...
```{r, echo = FALSE, eval = FALSE}
run_n <- function(n, eby = 3, eba = 1, se = 1){
  by <- rnorm(n = n, mean = eby, sd = se)
  ba <- rnorm(n = n, mean = eba, sd  = se)
  return(by/ba)
}
res <- sapply(c(1, 0.1, 0.05), function(s){run_n(100000, eby = 0, eba = 1, se = s)})
```
...
knitr::include_graphics("img/10_bandt_fig2.png")
```
</center>

```{r, echo = FALSE, eval = FALSE}
run_one <- function(n, a1, a2, b1, b2, sa, sy){
  df <- data.frame(Z = rbinom(n = n, size = 1, prob = 0.5), 
                   U= rnorm(n =n))
  df <- df %>% mutate( EA = a1*Z + a2*U, 
                   A = EA + rnorm(n = n, sd = sa), 
                   EY = b1*A + b2*U, 
                   Y = EY + rnorm(n, sd = sy))
  f1 <- lm(A~Z, data = df)
  f2 <- lm(Y~Z, data = df)
  bhat <- f2$coefficients[2]/f1$coefficients[2]
  f1e <- lm(EA~Z, data = df)
  f2e <- lm(EY~Z, data = df)
  deltau <- with(df, mean(U[Z == 1])-mean(U[Z == 0]))
  eq <- b1 + (b2*deltau/(a1 + a2*deltau))
  return(c(f1$coefficients[2], f2$coefficients[2], bhat, f2e$coefficients[2]/f1e$coefficients[2], eq))
}
res <- replicate(n = 500, run_one(1000, a1 = 2, a2 = 2, b1 = 0, b2 = 2, sa = 0.5, sy = 0.5))
```
...
$$E[\hat{\beta} - \beta] \approx \frac{\sigma_{\epsilon_{A}, \epsilon_{Y}}}{\sigma^2_{A}}\frac{K-2}{\tau^2}$$
- This approximation shows us that the bias of the IV estimator is approximately the bias of the OLS estimate divided by the F-statistic. 
...
- Another possible source of bias is selection bias. 
...
- Confounding from population structure and assortative mating.
  + We generally try to correct for this in the first stage of regression. 
...
- The exposure can be ill-defined.
  ...
  + We may know that modifications in genetic regulation of lipids affects disease risk but does that mean that if we pharmaceutically alter lipids later in life we can prevent the disease?
...
- Despite this long list of problems, MR has one big resource -- lots and lots of genetic variants. 
...
- We started this section with a very strong model that required complete homogeneity and linearity of effects for bht the $A$ and $Y$ model. 

- It turns out that we can relax this slightly. 

- The first equation need not be structural. For example, if $Z$ is a surrogate instrument, this equation only expresses an association, rather than a causal link. 
...
- We also don't need homogeneity in either equation, as long as we satisify 
$$Cov(Z_i, \epsilon_A) = 0 \qquad Cov(A_i, \epsilon_Y) = 0$$

- This first condition means that the structural form of the first equation needs to be correct -- the association between $Z$ and $A$ needs to be linear. 

- In the second equation, the true relationships does not need to be linear. 
  + If the true causal relationship is not linear, we will still estimate the ATE. 
  + However, the ATE will depend on the distribution of $A$ in our sample. 
...
```{r, echo = FALSE, eval = FALSE}
run_one <- function(n, f = function(z){z^2}, 
                    g = function(a){2*a}){
  df <- data.frame(Z = rnorm(n = n)) %>% 
        mutate(A = f(Z) + rnorm(n = n),
               Y = g(A) + rnorm(n = n))
  
  num <- lm(Y ~ Z, data = df) %>% with(., coefficients[2])
  denom <- lm(A ~ Z, data = df) %>% with(., coefficients[2])
  bhat <- num/denom
  return(bhat)
}

res <- replicate(n = 1000, run_one(1000))
```
