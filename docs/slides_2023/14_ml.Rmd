...
---
# 1. Targeted Minimum Loss Estimation of the ATE
...
- The problem is that, because ML estimators allow the model space to grow as the sample size grows, they converge to the true model too slowly to give good properties when plugged straight in to our existing methods.


- This issue will lead us to a new, double robust estimator that fixes the asymptotic convegence issue. 
...
- TMLE is also more stable than AIPW when $\hat{\pi}(L_i)$ are very small for some units. 
...
$$E[Y \vert A, L] = \hat{b}_0(A, L) + \epsilon H(A,L)$$

- If $\hat{b}_0$ is a great estimator of $Y$, then $\epsilon$ will be very small. If $\hat{b}_0$ is terrible, $\epsilon$ will be larger.

- Notice that this linear form looks a lot like the Bang and Robins estimator. The only difference is that in Bang and Robins, we fit $\epsilon$ and $\hat{b}_0$ simultaneously.
