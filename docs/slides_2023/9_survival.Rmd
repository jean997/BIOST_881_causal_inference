...
- If $D_k \ci \bar{C}_k$, we could estimate $E[D_k(\bar{C}_k = 0)]$ as $E[D_k \vert \bar{C}_k = 0]$ by counting the proportion of people who had died by time $t_k$ and who were not censored before time $t_k$. 

- However, this throws away information about everyone who was censored before $t_k$. 

---
## Kaplan-Meier Curves

- We can get a much better estimate of the survival by taking advantage of the product identity, 
$$S_{k} = \prod_{j = 1}^{k}\left(1 - h_j\right)$$
- If we can estimate $h_0, \dots, h_k$ then we can estimate $S(k)$.
...
- We can estimate the hazard, $h_k = P[D_k(\bar{C}_k = 0) = 1 \vert D_{k-1} = 0]$ if we assume that censoring before time $t_k$ to be independent of death during interval $(t_{k-1}, t_k]$.
