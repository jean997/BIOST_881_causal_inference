...
- This is called **weak instrument bias** because it occurs when the effect of the instrument on the exposure is small.
...
- With independent samples, $\epsilon_{Y,k}$ and $\epsilon_{A,k}$ are independents.
...
where $\kappa = \frac{1}{p}\sum_{k = 1}^K \frac{\mu_k}{\sigma^2_{AZ_k}}$ represents the average instrument strength. 
---
## Expectation of IVW Estimator
...
- When there is sample overlap then $\epsilon_{A,k}$ and $\epsilon_{Z_k}$ are not independent and there is an additional term in the expression for the expected bias.
...
- If each instrument explains a small amount of the variance of $A$ then $\rho$ is approximately equal to the population correlation of $A$ and $Y$ times the proportion of sample overlap. 

- If the sample is completely overlapping, the bias of the OLS estimate (regression $A$ on $Y$ directly) is $\rho c -\gamma$. 

-  So weak instrument bias is in the direction of the unadjusted (confounded) OLS estimate. 
