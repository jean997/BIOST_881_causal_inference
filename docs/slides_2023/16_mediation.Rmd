...
- This means that any effect of treatmenton $Y$ in these strata must be a direct effect. 
...
$$\beta = \frac{\sum_{c = 3}^4 \pi_c E[Y \vert C = c, X = x]}{\pi_3 + \pi_4}$$
...
- Gallop et al. then use Bayesian methods with uninformtive priors to estimate posterior means for $\beta_c$ ($c = 1, \dots, 4$). 

- Finally the average direct effect is a weighted average over the four strata.
