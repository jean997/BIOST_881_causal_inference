...
- Suppose we want to compare "always treat" and "never treat" strategies.
  ...
  + We then combine weights for time 0 treatment and weights for censoring. 
  $$\frac{1}{P[A_0 = 1]} \times \frac{1}{P[A_1 = 1 \vert A_0 = 1, L_1]}$$
...
- To estimate $E[Y(g)]$, we should limit ourselves only to units with $\bar{A}_i = (g_0(L_{0,i}), g_1(A_{0,i}, \bar{L}_{1,i}), \dots, g_K(\bar{A}_{k-1,i}, \bar{L}_{k,i}))$.
