...
- The trick is to predict $Y(a)$ for **all individual** with covariate data, not just those with non-missing outcomes. 
...
- With parametric models, the two methods rely on different assumptions and so are not the same.
...
- Some people suggest that we shouldn't be trying to estimate the countefactuals in these regions at all, since this requires extrapolating into regions with low density of data points. 
