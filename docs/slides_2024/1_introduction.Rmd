...
set.seed(47)
n = 20
high_risk <- rbinom(n = n, size = 1, prob = 0.5)
A <- rbinom(n = n, size = 1, prob = 0.2 + 0.5*high_risk)
Y_0  <- rbinom(n = n, size = 1, prob = 0.2 + 0.4*high_risk)
Y_1  <- rbinom(n = n, size = 1, prob = 0.2 + 0.4*high_risk - 0.1)

D <- data.frame(Ind = seq(n), Y_0, Y_1, A) %>% arrange(A)
...
Surgery example:
+ A person's chance of surviving after surgery is the same whether they chose the treatment type of their own accord or were assigned to the treatment.
...
+ To create this large *pseudo-population*, we each member of our trial and duplicate them (imagine two people identical in $L$).
One member of each pair recieves treatment $A = 1$ and the other recieves treatment $A = 0$. 
...
## Causal Effects from in Observational Data
...
- In some discrimination studies, there is a work-around: intervene on an observers perceptions:
