...
- Recall from L8, in G-estimation for a single time-point, we start by writing a semiparametric structural mean model for the contrast of interest, 

$$E[Y(a) \vert A = a, L=l]-E[Y(0)\vert A = a, L=l] = \gamma(a, l; \beta)$$
- We then define the $H$ function, $H_i(x) = Y_i - \gamma(A_i, L_i; x)$.

- Finally, we find the value of $x$ that makes $H(x)$ orthogonal to $A \vert L$ by either solving an estimating equation doing a grid search.


---
## G-Estimation

- Starting with the two time-point model, we will use the same strategy as in L8 but we will write two semiparametric structural mean models.

$$
\begin{split}
&E[Y(a_0, 0) - Y(0, 0) &\vert A_0 = a_0, L_0= l_0] = \gamma_0(a_0, l_0; \beta_0) = \beta_0 a_0\\
...
- Just as before, our goal is to find a value for $x$ that makes mean sequential exchangeability true. 
...
- $E[H_0(x^\star)] = E[Y(\bar{0})]$ at the true value of $x^\star$, so we can estimate $E[Y(\bar{0})]$ as the sample average of $H_{0, i}(\hat{x})$.
...
- Suppose we want to estimat $E[Y(a_0, a_1)]$. 
...
$$
\begin{split}
\hat{E}[Y(a_{0}, 0) ] &= \hat{E}_{L_0}[Y(a_0, 0) \vert L_{0} = l_0] \\
&= \hat{E}_{L_0}[Y(0, 0)] + E_{L_0}[\gamma_0(a_0, l_0; \hat{x})]\\
...
$$
\begin{split}
\hat{E}[Y(a_{0}, a_1) ] &= \hat{E}_{\bar{L}_1}[Y(a_0, a_1) \vert \bar{L}_1, A_{0}= a_0] \\
...
- Just as in G-computation, we don't have to limit our simulated data size to the orignal data size. We can resample many times.
...
dat3 <- generate_data(5000)
dat3_long <- dat3 %>%
            select(id, Z, A.0, A.1, A.2, L.0, L.1, L.2, Y) %>% 
            pivot_longer(cols = c("A.0", "A.1", "A.2", "L.0", "L.1", "L.2"), 
                         names_to = c(".value", "time"), 
                         names_sep = "\\.")
dat3_long$time <- as.numeric(dat3_long$time)

dat3.summ <- dat3 %>% group_by(A.0, A.1, A.2) %>% summarize(meanY = mean(Y), N= n())
dat3.summ <- dat3.summ[c(1, 4, 8),]
dat3.summ$Counterfactual <- c(mean(dat3$EY000), mean(dat3$EY011), mean(dat3$EY111))
t <- kable(dat3.summ, col.names = c("A0", "A1",  "A2", "MeanY", "N", "MeanCounterfactualY"), 
...
f0 <- glm(A.0 ~ L.0, family = "binomial", data = dat3)
f1 <- glm(A.1 ~ A.0 + L.1, family = "binomial", data = dat3)
f2 <- glm(A.2 ~ A.1 + L.2, family = "binomial", data = dat3)

## For numerator of stabilized weights
s0 <- glm(A.0 ~ 1, family = "binomial", data = dat3)
s1 <-  glm(A.1 ~ A.0 , family = "binomial", data = dat3)
s2 <- glm(A.2 ~ A.1, family = "binomial", data = dat3)
```
...
```{r}
dat3$pA.0 <- predict(f0, type = "response")
dat3$pA.1 <- predict(f1, type = "response")
dat3$pA.2 <- predict(f2, type = "response")
dat3$sA.0 <- predict(s0, type = "response")
dat3$sA.1 <- predict(s1, type = "response")
dat3$sA.2 <- predict(s2, type = "response")

dat3 <- dat3 %>% mutate(W.0 = case_when(A.0 == 1 ~ sA.0/pA.0, 
                               TRUE ~ (1-sA.0)/(1-pA.0)),
...
ipw_ests <- data.frame(A = c("0-0-0", "0-1-1", "1-1-1"), 
                       Truth = round(dat3.summ$Counterfactual, digits = 2), 
...
gform_result <- gformula(obs_data = dat3_long, id = id,
...
dat3_long$time <- dat3_long$time+ 1

dat3_gest <- FormatData(
  data = data.frame(dat3_long),
...
outcomemodels <- list("Y~A + L + A*L", 
                      "Y~A + L + A*L", 
                      "Y~A + L + A*L")
gest_result <- gestSingle(dat3_gest, idvar, timevar, Yn, An, 
...
dat3$H2 <- with(dat3, Y - g2(A.2, L.2))
dat3$H1 <- with(dat3, H2 - g1(A.1, L.1))
dat3$H0 <- with(dat3, H1 - g0(A.0, L.0))
mean(dat3$H0)
```
...
gest_mean <- function(a0, a1, a2, nrep = 10){
 sim_dat <- purrr::map_dfr(1:nrep, function(i){
    ...
    n <- nrow(simdat3)
    simdat3$A.0 <- a0
    simdat3$L.1 <- with(simdat3, 
                        rl1(nsamp = n, A.0 = A.0, L.0 = L.0))
    simdat3$A.1 <- a1
    simdat3$L.2 <- with(simdat3, 
                        rl2(nsamp = n, A.1 = A.1, L.1 = L.1))
    simdat3$A.2 <- a2
    simdat3$Y <- with(simdat3, H0 + g0(A.0, L.0) + 
                                    g1(A.1, L.1) + 
                                    g2(A.2, L.2))
    return(simdat3)
  })
...
gestfunc <- function(dat){
  ...
  gest_mean <- function(a0, a1, a2, nrep = 10){
   sim_dat <- purrr::map_dfr(1:nrep, function(i){
      simdat3 <- dat
      simdat3$A.0 <- a0
      simdat3$L.1 <- rl1(nsamp = nrow(simdat3), A.0 = simdat3$A.0, L.0 = simdat3$L.0)
      simdat3$A.1 <- a1
      simdat3$L.2 <- rl2(nsamp = nrow(simdat3), A.1 = simdat3$A.1, L.1 = simdat3$L.1)
      simdat3$A.2 <- a2
      simdat3$Y <- with(simdat3, H0 + g0(A.0, L.0) + g1(A.1, L.1) + g2(A.2, L.2))
      return(simdat3)
    })
    return(mean(sim_dat$Y))
  }
...
fit_ice_stratified <- function(a0, a1, a2, dat){
  ...
  dat$Q3 <- predict(Q3mod, newdata = dat3, type = "response")
...
```{r}
c(fit_ice_stratified(0, 0, 0, dat3), 
  fit_ice_stratified(0, 1, 1, dat3),
 fit_ice_stratified(1, 1, 1, dat3))
c(fit_ice_nonstratified(0, 0, 0, dat3),
  fit_ice_nonstratified(0, 1, 1, dat3),
  fit_ice_nonstratified(1, 1, 1, dat3))
```
...
rr <- c(fit_ice_stratified(0, 0, 0, dat3),
                fit_ice_stratified(0, 1, 1, dat3),
                fit_ice_stratified(1, 1, 1, dat3), 
        fit_ice_nonstratified(0, 0, 0, dat3),
                fit_ice_nonstratified(0, 1, 1, dat3),
                fit_ice_nonstratified(1, 1, 1, dat3))
