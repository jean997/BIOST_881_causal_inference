...
- This model is **semiparametric** because we don't specify a functional for for $E[Y(0) \vert L]$.
...
- If you knew $H(x)$, we could fit a regression like
$$
logit \left( P[A = 1 \vert H(x), L] \right) = \alpha_0 + \alpha_1 H(x) + \alpha_2 L
$$

- If $\alpha_1$ is equal to 0, we could conclude that $H(x) \ci A \vert L$. 

- We could find the value of $x$ that makes this true by doing a grid search, repeatedly fitting the regression and choosing the value of $x$ that gives $\hat{\alpha}_1(x)$ closest to 0. 

---
## G-Estimation

- Alternatively, we know that if $H(x) \ci A \vert L$ then $E[H(x) (A - E[A \vert L])] = 0$.

- So we have an estimating equations formulation. 

- We want to find the value of $x$ such that 

$$\sum_{i = 1}^n  H_i(x)(A_i - E[A \vert L_i]) = 0$$

- If $H(x) = Y_i - xA_i$, this is solved by 

$$x^* = \frac{\sum_{i=1}^n Y_i (A_i - E[A \vert L_i])}{\sum_{i=1}^n  A_i (A_i - E[A \vert L_i])}$$

- This will give the same answer as the grid search method if we use the same model for $E[A \vert L]$.
...
- Using the grid search method, this means using $W^{C}$ as weights when we fit the model for $P[A \vert H(x), L]$.

- Using the estimating equations version, we add weights to the sums in numerator and denominator,

$$x^* = \frac{\sum_{i: C = 0} W^C_i Y_i (A_i - E[A \vert L_i])}{\sum_{i:C = 0}^n W^C_i A_i (A_i - E[A \vert L_i])}$$
...
$$\sum_{i = 1}^n 1_{C_i = 0}W^C_i H_i(x^*)(d(A_i) - d(E[A \vert L_i])) = 0$$ where $d$ is a function of $A$ and $L$ with dimension equal to the dimension of $x$. 
...
- If $H(x)_i = Y_i - xA_i$, then $H(x) - E[H(x) \vert L] = (Y_i - \hat{Y}_i) - xA_i$.
...
- We then obtain a function $H(x)$ such that $E[H(x^*)] = Y(0)$ for some value of $x^*$.
