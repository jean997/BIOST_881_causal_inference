---
title: "L14: Machine Learning in Causal Inference"
author: "Jean Morrison"
institute: "University of Michigan"
date: "Lecture on 2024-04-01\n (updated: `r Sys.Date()`)"
output:
...
$$\Psi_a(P_0) \equiv E[Y(a)] = \int E[Y \vert A = a, L = l]dF_l(l)$$
is
...
- Want $\hat{\Psi}(P_n)$ such that

$$\hat{\Psi}(P_n) = \Psi(P_0) + \frac{1}{n}\sum_{i = 1}^n EIF(O_i) + o_p(n^{-1/2})$$
$$\hat{\Psi}(P_n) = \frac{1}{n}\sum_{i = 1}^n \frac{1_{A_i = a}}{\pi(a, L_i )}\left(Y_i - b(a, L_i) \right) + b(a, L_i) + o_p(n^{-1/2})$$

- Plugging in $\hat{b}$ and $\hat{\pi}$
...
- We use the logistic version of the one step update is that it guarantees an estimate of $E[Y(a)]$ that is between 0 and 1. 
...
- Let $\hat{\psi}$ be the TMLE estimate of the ATE. The influence function for the TMLE estimate is
...
- The key to figuring out if the estimator is consistent is to assess what happens to the remainder term, 

$$R_n = \hat{\Psi}(P_n) - \Psi(P_0) - \frac{1}{n}\sum_{i = 1}^n EIF(O_i)$$

- To have asymptotic linearity, we needed this term to be $o_p(n^{-1/2}).$

- For both AIPW and TMLE, $R_n$ is proportional to $(b(a, L_i)-\hat{b}(a, L_i))(\hat{\pi}(a, L_i) - \pi(a, L_i))$, the product of the error in $\hat{b}$ and the error in $\hat{\pi}$.
 
---
## Double Roubst Estimation Properties

- Since the remainder term is a product of the error in the two estimators,
...
- Benkesser et al. (2017) and van der Laan (2014) propose an extension of TMLE that is $\sqrt{n}$-consistent if either the outcome model estimator or the PS estimator are consistent, DR-TMLE
...
- Rather than using a single update step, DR-TMLE uses an iterative procedure.

---
## Multi-Step TMLE

- Simulations from Benkesser et al. (2017). 
  - Only the outcome regression is estimated consistently. 
  - $\hat{b}$ has slower than $\sqrt{N}$ convergence.
...
- Super Learner puts most of its weight on the linear regression, but also gives weight to the other learners.
