---
title: "L13: Time to Event Outcomes"
author: "Jean Morrison"
institute: "University of Michigan"
date: "Lecture on 2026-03-31\n(updated: `r Sys.Date()`)"
output:
...
- In the time to event setting, it is unlikely that we know the distribution of $T$ in the population. 
...
$$
\begin{split}
P[D_2(C_1 = 0, C_2 = 0) = 0] =& 1 - (P[D_1 = 1] + P[D_2 = 1 \vert D_1 = 0]P[D_1 = 0]) \\
 = & 1-(0.1 + 0.05\cdot 0.9) = 0.855
\end{split}
$$
...
- We computed $P[D_2(\bar{C}_2 = 0)]$ as 
$$
\begin{split}
P[D_2(\bar{C}_2 = 0)]  =& 1 - (P[D_1 = 1] + P[D_2 = 1 \vert D_1 = 0]P[D_1 = 0]) \\
= & 1 - P[D_1 = 1] - P[D_2 = 1 \vert D_1 = 0](1-P[D_1 = 1])\\
= & (1-P[D_1 =1])(1 - P[D_2 = 1 \vert D_1 = 0])
\end{split}
$$

- Because death and censoring are independent $P[D_2 = 1 \vert D_1 = 0] = P[D_2 = 1 \vert D_1 = 0, C_2 = 0]$, so both of these quantities are observable in our data. 

- We are using the product identity 
$$S_{k} = \prod_{j = 1}^{k}\left(1 - h_j\right)$$
---
...
- We can use the same IP weighting + marginal structural model approach we used in L6 for non-time to event data. 
...
---
## G-Computation for Time-to-Event Data

- Recall standardization from L6: 
...
- In this case, we need a parametric (or semi-parametric) model for the hazard conditional on $A$ and confounders $L$. 

- We then compute the conditional survival from the conditional hazard using the product trick setting everyone's $A$ to 0 or to 1. 
...
- This is the same as the models we saw in L9 except that here we are using the log link. 

---
## G-estimation

- As in L9, if we think there is effect modification, we should include it in the model.
...
- In the NHEFS data, using the model $$T_i(A = 0) = T_i \exp(\psi A_i)$$ the resulting estimate of $\psi$ is  $-0.05 (95\%\ \text{CI:} -0.22 \text{ to } 0.33)$.
  + Our point estimate is that quitting smoking extends expected survival time by about $5\%$ ($1.05 \approx \exp(0.05)$), however we do not find evidence to reject that the true causal effect is zero.
...
+ If there are common causes of treatment and $Y_{k-1}$ or of $Y_k$ and $Y_{k - 1}$, conditioning on $Y_{k-1}$ will introduce selection bias. 
