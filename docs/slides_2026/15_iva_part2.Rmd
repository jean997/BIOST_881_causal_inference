...
- Using the ratio framework, we use Sample 1 to estimate $\hat{\beta}_{AZ}$ and Sample 2 to estimate $\hat{\beta}_{YZ}$. 
...
$$\hat{\beta}_{IV} = \frac{\gamma \mu + \sigma_{YZ} T_Y}{\mu + \sigma_{AZ} T_A} = \gamma \frac{1}{1 + (\sigma_{AZ}/\mu) T_{AZ}} + \frac{(\sigma_{YZ}/\sigma_{AZ}) T_{YZ}}{(\mu/\sigma_{AZ}) + T_{AZ}}$$

- If $\mu$ is not equal to zero and $T_{YZ}$ and $T_{AZ}$ are independent (the samples do not overlap), then the last term converges to zero almost surely as the sample size for the two regression stages go to zero. 
  + As long as the ratio $\sigma_{YZ}/\sigma_{AZ}$ is bounded.

- As $\sigma_{AZ}$ gets small, $\frac{1}{1 + (\sigma_{AZ}/\mu) T_{AZ}} \to 1$. 

- As the sample size used to estimate $\hat{\beta}_{AZ}$ increases, $\hat{\beta}_{IV} \to \gamma$ . 
