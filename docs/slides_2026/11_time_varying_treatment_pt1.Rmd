...
- We can think of $E[Y(0, 0)]$ as $E[Y(A_0 = 0, \text{adherent})]$ and $E[Y(1, 1)]$ as $E[Y(A_0 = 1, \text{adherent})]$. where adherence means staying on the same treatment at time 1. 

- We first compute the censoring weights

$$W^{C}_i = \frac{1}{P[A_1 = A_{0,i} \vert A_0 = A_{0,i}, L_1, L_0]}$$

- Then we compute the confounding weights for treatment at time 0:

$$W_i^{L} = \frac{1}{P[A_0 = A_i \vert L_0]}$$

- The total weights are the product of $W^{L}$ and $W^{C}$.
...
- We can see that, conditional on $A_0$ and $L_1$, treatment choice at $A_1$ doesn't affect the expected outcome, so we should find that $E[Y(g)] - E[Y(g^{\prime})] = 0$.
...
- Using the $W^{\bar{A}}$ weights, we can compute $E[Y(g)]$ and $E[Y(g^\prime)]$ using the non-parametric approach. 
...
- We will estimate $E[Y(0, 0, 0)]$, $E[Y(0, 1, 1)]$ and $E[Y(1, 1, 1)]$
...
- We need a propensity model for each time point.
- Below we fit a model for each time point separately.
...
- If we wanted to share coefficients across time points, we could fit a single model combining time points. 

```{r}
sim_dat_long <- select(sim_dat, id, Z, L.0, A.0, L.1, A.1, L.2, A.2) %>% 
    pivot_longer(
        cols = matches("^[LA]\\.\\d$"),
        names_to = c(".value", "time"),
        names_pattern = "([LA])\\.(\\d)"
    ...
    arrange(id, time) %>%
    group_by(id) %>%
    mutate(Aminus1 = lag(A, default = 0)) %>%
    ungroup() %>%
    select(id, Z, time, L, A, Aminus1)

library(geepack)
f_all <- geeglm(A ~ L + I(Aminus1*as.numeric(time > 0)), family = "binomial", 
                id = id, corstr = "independence", data = sim_dat_long)
f_all2 <- geeglm(A ~ L*time + I(Aminus1*as.numeric(time > 0)), family = "binomial", 
                id = id, corstr = "independence", data = sim_dat_long)
```
...
f_msm2 <- lm(Y ~ A.0*A.1 + A.1*A.2 + A.0*A.2,  weights = W, data = sim_dat)
...
- For all estimates the propensity score models are correct. 
- For estimate 3, the MSM is incorrect. 
...
- To estimate $E[Y(a)]$ we replaced each person's treatment value with $a$ and then estimated $\hat{Y}_i(a) = \hat{E}[Y \vert A = a_0, L = L_i]$. 
...
---
## Parametric G-Formula in Simulations

- First we specify arguments for `gfoRmula`.
...
- The `gfoRmula` package will perform boostrapping for us. This can take a bit of time.
...
- It may be impossible to correctly specify the parametric G-formula even when the strict null does not hold, for example in the graph below under our previous data generating model.
