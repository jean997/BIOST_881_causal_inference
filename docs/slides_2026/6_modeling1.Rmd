...
$$W_i = \begin{cases} 1/\pi_i \qquad & A_i = 1\\ 1/(1-\pi_i) \qquad & A_i = 0\end{cases}$$
---
...
- If you have survey data that come with survey weights, be sure to read about how these are calculated. 
  + If you want to estimate a parameter for the larger population, you will need to use the survey weights. 
...
- In other words, the coefficients for terms including $A$ are structural, and it is ok to interpret them this way. 

---
## Outcome Modeling

- Our model includes an interaction between $A$ and smoking intensity. 

- If the model had included only a main effect for $A$, the coefficient on $A$ would be the marginal causal effect.
  + This is **only** true for linear (identity link) models.
...
- In the model we fit, the coefficient on $A$ is the average treatment effect within strata of smoking intensity. 
...
- If the set of variables in our model for $E[Y \vert A, L]$ satisfy the s-backdoor criterion, then we don't need to do anything extra to adjust for censoring.


- Recall that the s-Backdoor method requires us to average over the uncensored distribution of covariates.

- This means we need to predict $Y(a)$ for *all individuals*, not just those with non-missing outcomes. 
...
- To compute the variance of the effect estimate, we need to bootstrap. 
...
- In either case, if the parametric models are wrong, the causal estimators will be biased. 
...
- However, G-Computation can be more precise than IP weighting in some circumstances. 
  ...
  + IP weighting is inefficient when weights are very large. 
