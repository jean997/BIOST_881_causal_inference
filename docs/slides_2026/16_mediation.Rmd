...
$$
\begin{split}
&E[Y(A = 1, M = m)] = \beta_{Y,M} m + \beta_{Y,A} + \beta_{Y, MA} m\\
&E[Y(A = 0, M = m)] = \beta_{Y,M} m\\
...
$$
\begin{split}
&E[Y(A = 1, M = M(a))] = \beta_{Y,M} \beta_{M,A} a + \beta_{Y,A} + \beta_{Y, MA} \beta_{M,A} a\\
&E[Y(A = 0, M = M(a))] = \beta_{Y,M} \beta_{M,A} a\\
...
$$
\begin{split}
&E[Y(A = a, M = M(1))] = \beta_{Y,M} \beta_{M,A}  + \beta_{Y,A} a + \beta_{Y, MA} \beta_{M,A} a\\
&E[Y(A = a, M = M(0))] =  \beta_{Y,A} a \\
...
- There are some assumptions under which we *can* identify the NDE and NIE. 
...
- If the NDE and NIE are identifiable, we can use a variation of the g-formula. 
...
- Since $E[Y(a,m)]$ is a joint counterfactual, so all of the strategies we learned for time-varying treatments apply.

- G-computation, IP weighting, G-estimation with structural nested mean models, and ICE estimation are all valid approaches. 
...
- These factors may push us to using G-estimation with structural nested mean models or ICE estimation. 
...
Stratum 1 (formerly Compliers): Units with $M_i(1) = 1$ and $M_i(0) = 0$

Stratum 2 (formerly Defiers): Units with $M_i(1) = 0$ and $M_i(1) = 1$

Stratum 3 (formerly Always Takers): Units with $M_i(1) = M_i(0) = 1$

Stratum 4 (formerly Never Takers): Units with $M_i(1) = M_i( 0) = 0$
...
- There are many principal stratification estimation methods. 
...
- For any unit, we get to observe $A$ and $M$ but not their principal stratum. However, seeing $A$ and $M$ narrows the possibilities to two. 

- For example, if $A_i = 0$ and $M_i = 1$, we know the unit is in Stratum 2 (defiers) or Stratum 3 (always takers).
