...
---
## G-Estimation in Simulations

```{r, echo = FALSE, message = FALSE, warning = FALSE}
library(dplyr)
library(tidyr)
library(kableExtra)
sim_dat <- readRDS("img/11_sim_dat.RDS")
...
all_ests <- readRDS("img/11_all_ests.RDS")
```
...
- It does not calculate $E[Y(a)]$ for us. This step requires simulating covariates, so it is not trivial. 
...
- I bootstrapped the standard error (code hidden but you can find it [here](https://github.com/jean997/BIOST_881_causal_inference/blob/master/docs/slides_2026/12_time_varying_treatment_pt2.Rmd)).
...
$$\hat{Q}_2(L_1, L_0) =  E[\hat{Q}_3 \vert \bar{L}_{1}, \bar{A}_{1} = \bar{a}] = \theta_{0, 1} +  \theta_{1,1} L_{1} + \theta_{2,1}L_0$$
- We can fit this using linear regression limited to those with $\bar{A}_1 = \bar{a}$.
...
- Now plug $\hat{Q}_2(L_1, L_0)$ into the expression 
...
- Next, estimate the blue part. We can use a regression model with $\hat{Q}_2$ as the outcome and $L_0$ as predictors, fit using only those with $A_0 = a_0$.

- To find the outcome, we compute $\hat{Q}_{2,i}$ for each person with $A_{0,i} = a_0$ by plugging $L_{1,i}$, $L_{0,i}$ into the fitted model from the previous stage.
...
---
## Non-Stratified ICE Estimation

- At step $k$, we fit the model only among individuals with treatment history matching our target treatment history up to time $k$. 

- This strategy is non-parametric with respect to the $Y-A$ relationship. 

- In other words, we are not borrowing information from people who have similar treatment histories. 

- Alternatively, we could include the treatment variables in the models and fit a fully parametric model using everyone in the data at every step. 
  + This may be more powerful but requires stronger assumptions.

---
## ICE Estimation

- The ICE estimator is valid if the outcome regression is correct at each time point. 

- Unlike parametric G-formula and G-estimation, we don't have to estimate the joint distribution of $\bar{L}$. 



---
## Doubly Robust ICE Estimation


- We can turn the ICE estimator into a doubly robust estimator by adding the Scharfstein et al "special covariate" to each regression. 

- For a point treatment, the Scharfstein et al method for estimating $E[Y(a)]$ is

1. Estimate $P[A = a \vert L]$ and compute $W^{A}_i  = \frac{1}{\hat{P}[A = A_i \vert L_i]}$ 
...
---
## ICE Estimation in Simulations

- I implemented this estimator by hand, but it was straight-forward.

- I used both the stratified and non-stratified methods.

- Recall, in the stratified method, the regression at stage $k$ is fit only amongst those with the desired treatment history up to time $k$.

- In the non-stratified method, the outcome model is fully specified including the $Y-A$ relationship and fit using the full data. 
...
- If our goal is to estimate $E[Y_k(\bar{A}_k)]$, one option is treat each outcome separately. 

- For each $k$, consider a study that ends at time $k$ with a single outcome $Y_k$. 

- All previous previous values of $Y$ can be treated as confounders. 

- We can use our existing machinery (e.g. IP weighting, G-estimation, ICE, etc.) to estimate $E[Y_k(\bar{A}_k)]$


---
## Combining Information Across Time

- In some cases, it makes sense to combine information across time-points.

- This is possible when the marginal structural model at every time point is the same.

- We could potentially include some effect modification by time to allow some parameters to vary with time. 
...
$$E[Y_{k}(\bar{A}_k)] = o\beta_{0,j} +  \beta_1 A_k + \beta_2 A_{k-1}$$
- In this MSM, we have a time-specific intercept but the causal differences are shared across times. 

- We are assuming that the effect of changing from a treatment course with the last two time points untreated to a treatment course with the final time point treated is the same at every time point. 

- Because our MSM includes the last two treatments, we can only fit this model for $k \geq 1$. We could either fit a separate model for $Y_0$, ignore this outcome, or assume that $A_{-1} = 0$. 
...
- If we had a study that only included two time-points and one measured outcome at time 1,we would estimate the parameters of the MSM using IP weighting for time-varying treatments (L10). 
...
- To estimate a common $\beta_1$ and $\beta_2$ shared across time-points, we use the same procedure. 

- Think of our data as $K$ shorter studies that all include the same people. 
...
- They include these variables in the MSM because they want to use them in the numerator of the stabilized weights. Since the MSM is conditional on $V_{i,0}$, they are assuming no effect modification by $V_{i,0}$.
...
- The analysis now also relies on the assumption of non-informative censoring, meaning people with high viral loads were not more (or less) likely to be censored than other people with the same covariate and treatment history. 
