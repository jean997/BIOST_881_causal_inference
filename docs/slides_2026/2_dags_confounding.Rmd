...
1. Using Properties of DAGs to Identify Conditional Exchangeability.
...
+ We can represent causal relationships with graphs. 
...
- For many people, the graph encoding is fairly intuitive. 
  - Graphs can be a useful tool for communicating structural assumptions across domains. 
...
elements of $E$ are un-ordered. 
  - Our graphs will always be directed. 
...
- Two nodes are *connected* if there is at least one path between them. Otherwise, they are *disconnected*.
...
- Intervening on the readout on the machine does not change the patients blood pressure.
...
- An equation is structural if the lefthand term can be interpreted as the counterfactual value when the terms on the right are intervened on.
...
# 3. Using Properties of DAGs to Identify Conditional Exchangeability.
...
- We want to know if the outcome, $O$, is exchangeable with respect to the treatment, $T$. That is, we want to know if $O(t) \ci T$.

- If not, what set of variables, $L$ can we condition on such that $O(t) \ci T \mid L$?
...
+ But **conditioning on a collider** introduces an association between $A$ and $Y$. 
  - This may not be as intuitive as the bias introduced by a common cause. 
...
- In the graph below, $L_2$ satisfies all of these criteria but is not a confounder.
...
- The factorization result allows us to conclude that d-separation in the SWIG implies conditional exchangeability. 
  ...
  - Since $Y(a)$ is a node in the SWIG, we can now read conditional exchangeability off of the graph using d-separation.

- Achieving $d$-separation in the SWIG is equivalent to satisfying the backdoor criterion. 

- So the factorization result also implies the FFRCISTG assumptions are sufficient for the backdoor backdoor criterion theorem to hold.
