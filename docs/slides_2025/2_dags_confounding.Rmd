...
$$ A \text{ is } d\text{-separated from }B\text{ given }C \Rightarrow A \ci B \vert C$$ 
...
- It does not tell us about conditional exchangeability, which is conditional independence of a counterfactual value and a variable in the graph, e.g. $Y(A=a) \ci A \vert L$. 
...
- The factorization result allows us to conclude that d-separation in the SWIG implies conditional exchangeability. 
  - Recall: Conditional exchangeability says that $Y(a) \ci A \vert L$.
