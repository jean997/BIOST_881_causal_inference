...
- If there had been no censoring, could we identify $E[Y(a)]$? (i.e. is there a set of variables $\mathcal{V}$ such that $Y(a) \ci A \mid \mathcal{V}$)?
...
- If there is no effect of selection on $Y$, the node $Y$ could be written as $Y(C = 0)$, the value of $Y$ that we would observe if nobody was censored.
